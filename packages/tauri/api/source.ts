...
async function resolveFilepath(bundleName: string): Promise<string> {
  const path = await invoke<string>('plugin:wvb-tauri|source_resolve_filepath', { bundleName });
  return path;
}

async function getBuiltinBundleFilepath(bundleName: string, version: string): Promise<string> {
  const path = await invoke<string>('plugin:wvb-tauri|source_get_builtin_bundle_filepath', {
    bundleName,
    version,
  });
  return path;
...
async function getRemoteBundleFilepath(bundleName: string, version: string): Promise<string> {
  const path = await invoke<string>('plugin:wvb-tauri|source_get_remote_bundle_filepath', {
    bundleName,
    version,
  });
  return path;
...
async function loadBuiltinMetadata(
  ...
  const metadata = await invoke<BundleManifestMetadata | null>(
    'plugin:wvb-tauri|source_load_builtin_metadata',
    { bundleName, version }
  );
  return metadata;
}
...
async function loadRemoteMetadata(
  ...
  const metadata = await invoke<BundleManifestMetadata | null>(
    'plugin:wvb-tauri|source_load_remote_metadata',
    { bundleName, version }
  );
  return metadata;
}
...
async function unloadDescriptor(bundleName: string): Promise<boolean> {
  const removed = await invoke<boolean>('plugin:wvb-tauri|source_unload_descriptor', {
    bundleName,
  });
  return removed;
}
...
async function removeRemoteBundle(bundleName: string, version: string): Promise<boolean> {
  const removed = await invoke<boolean>('plugin:wvb-tauri|source_remove_remote_bundle', {
    bundleName,
    version,
  });
  return removed;
...
async function remoteRetainedVersions(bundleName: string): Promise<string[]> {
  const versions = await invoke<string[]>('plugin:wvb-tauri|source_remote_retained_versions', {
    bundleName,
  });
  return versions;
}
...
async function pruneRemoteBundles(bundleName: string): Promise<string[]> {
  const removed = await invoke<string[]>('plugin:wvb-tauri|source_prune_remote_bundles', {
    bundleName,
  });
  return removed;
}
