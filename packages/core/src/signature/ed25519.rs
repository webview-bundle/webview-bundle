...
impl Ed25519 {
  pub fn from_public_key_bytes(bytes: &[u8; 32]) -> crate::Result<Self> {
    let key = VerifyingKey::from_bytes(bytes).map_err(crate::Error::invalid_signature_key)?;
    Ok(Self { key })
  }
...
mod tests {
  ...
  async fn verifies_base64_encoded_signature() {
    ...
    assert!(verifier.verify(message, &signature_b64).await.is_ok());
    // Wrong message must not verify.
    assert_eq!(
      verifier
        .verify(b"tampered", &signature_b64)
        .await
        .unwrap_err()
        .code(),
